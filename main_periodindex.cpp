...
void usage()
{
    ...
    cerr << "USAGE" << endl;
    cerr << "       ./query_periodindex.exec [OPTION]... [DATA] [QUERIES]" << endl << endl;
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "?hvq:l:p:r:")) != -1)
    {
        switch (c)
        {
            case '?':
    ...
    // Build index
    tim.start();
    idxR = new AdaptivePeriodIndex(R, settings.numLevels, settings.numPartitions);
    ...
    size_t sumQ = 0;
    if (settings.verbose)
        cout << "Query\tMethod\tLevels\tPartitions\tResult\tTime" << endl;
    while (fQ >> qstart >> qend)
    {
        ...
        for (auto r = 0; r < settings.numRuns; r++)
        {
            ...
            if (settings.verbose)
                cout << "[" << qstart << "," << qend << "]\t" << strQuery << "\t" << settings.method << "\t" << settings.numLevels << "\t" << settings.numPartitions << "\t" << queryresult << "\t" << querytime << endl;
        }
    ...
    cout << "Avg partition size        : " << idxR->avgPartitionSize << endl;
