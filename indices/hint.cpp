...
void HINT::insert(const Record &rec)
{
    int level = 0;
    Timestamp a = rec.start >> (this->maxBits-this->numBits);
    Timestamp b = rec.end   >> (this->maxBits-this->numBits);
    Timestamp prevb;
    int firstfound = 0;
    ...
    while (level < this->height && a <= b)
    {
        ...
        { //last bit of a is 1
            if (firstfound)
            {
                this->pReps[level][a].push_back(rec.id);
//                this->pReps_sizes[level][a]++;
            }
            else
            {
                this->pOrgs[level][a].push_back(rec.id);
//                this->pOrgs_sizes[level][a]++;
                firstfound = 1;
            }
            //a+=(int)(pow(2,level));
        ...
        { //last bit of b is 0
            ...
            if ((!firstfound) && b < a)
            {
                this->pOrgs[level][prevb].push_back(rec.id);
//                this->pOrgs_sizes[level][prevb]++;
            }
            else
            {
                this->pReps[level][prevb].push_back(rec.id);
//                this->pReps_sizes[level][prevb]++;
            }
        }
        a >>= 1; // a = a div 2
...
void HINT::remove(const Record &rec)
{
    Timestamp a = rec.start;
    Timestamp b = rec.end;
    ...
    for (auto l = 0; l < this->numBits; l++)
    {
        ...
        // Handle the partition that contains a.
        for (auto iter = this->pOrgs[l][a].begin(); iter != this->pOrgs[l][a].end(); iter++)
            if(*iter == rec.id)
                *iter = 0;
        ...
        for (auto j = a+1; j <= b; j++)
        {
            for (auto iter = this->pReps[l][j].begin(); iter != this->pReps[l][j].end(); iter++)
                if(*iter == rec.id)
                    *iter = 0;
        }
        a >>= 1; // a = a div 2
    ...
    // Handle root.
    for (auto iter = this->pOrgs[this->numBits][0].begin(); iter != this->pOrgs[this->numBits][0].end(); iter++)
            if(*iter == rec.id)
                *iter = 0;
}
