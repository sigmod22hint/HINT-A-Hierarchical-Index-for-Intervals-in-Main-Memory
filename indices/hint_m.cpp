...
void HINT_M_SubsSopt::remove(const Record &rec)
{
    Timestamp a = rec.start >> (this->maxBits-this->numBits); // prefix
    Timestamp b = rec.end   >> (this->maxBits-this->numBits); // prefix
    Record qdummyE(0, rec.end+1, rec.end+1);
    RecordStart qdummySE(0, rec.end+1);
    RecordEnd qdummyS(0, rec.start);
    bool foundzero = false;
    bool foundone = false;
    ...
    for (auto l = 0; l < this->numBits; l++)
    {
        ...
        // Handle the partition that contains a: consider both originals and replicas
        for (auto iter = this->pOrgsIn[l][a].begin(); iter != this->pOrgsIn[l][a].end(); iter++)
            if(iter->id == rec.id)
                iter->id = 0;
        
        for (auto iter = this->pOrgsOut[l][a].begin(); iter != this->pOrgsOut[l][a].end(); iter++)
            if(iter->id == rec.id)
                iter->id = 0;
        ...
        for (int j = a+1; j <= b; j++)
        {
            for (auto iter = this->pRepsIn[l][j].begin(); iter != this->pRepsIn[l][j].end(); iter++)
                if(iter->id == rec.id)
                    iter->id = 0;
            for (auto iter = this->pRepsOut[l][j].begin(); iter != this->pRepsOut[l][j].end(); iter++)
                if((*iter) == rec.id)
                    (*iter) = 0;
        }
    ...
    // Handle root.
    for (auto iter = this->pOrgsIn[this->numBits][0].begin(); iter != this->pOrgsIn[this->numBits][0].end(); iter++)
        if(iter->id == rec.id)
            iter->id = 0;
}


void HINT_M_SubsSopt::insert(const Record &r)
{
    int level = 0;
    Timestamp a = r.start >> (this->maxBits-this->numBits);
    Timestamp b = r.end   >> (this->maxBits-this->numBits);
...
void HINT_M_ALL::scanFirstPartitionDelete_OrgsIn(unsigned int level, RecordId id, Timestamp a, Timestamp qstart, PartitionId &next_from)
{
    vector<pair<Timestamp, Timestamp> >::iterator iter, iterStart, iterEnd;
    RelationIdIterator iterI;
    tuple<Timestamp, RelationIdIterator, vector<pair<Timestamp, Timestamp> >::iterator, PartitionId> qdummy;
...
void HINT_M_ALL::scanFirstPartitionDelete_OrgsOut(unsigned int level, RecordId id, Timestamp a, PartitionId &next_from)
{
    RelationIdIterator iter, iterStart, iterEnd;
    tuple<Timestamp, RelationIdIterator, vector<Timestamp>::iterator, PartitionId> qdummy;
    vector<tuple<Timestamp, RelationIdIterator, vector<Timestamp>::iterator, PartitionId> >::iterator iterIO, iterIOStart, iterIOEnd;
...
void HINT_M_ALL::scanPartitionsDelete_RepsIn(unsigned int level, RecordId id, Timestamp a, PartitionId &next_from)
{
    RelationIdIterator iter, iterStart, iterEnd;
    tuple<Timestamp, RelationIdIterator, vector<Timestamp>::iterator, PartitionId> qdummy;
    vector<tuple<Timestamp, RelationIdIterator, vector<Timestamp>::iterator, PartitionId> >::iterator iterIO, iterIO2, iterIOStart, iterIOEnd;
...
void HINT_M_ALL::scanPartitionsDelete_RepsOut(unsigned int level, RecordId id, Timestamp a, PartitionId &next_from)
{
    RelationIdIterator iter, iterStart, iterEnd;
    tuple<Timestamp, RelationIdIterator, PartitionId> qdummy;
    vector<tuple<Timestamp, RelationIdIterator, PartitionId> >::iterator iterIO, iterIOStart, iterIOEnd;
...
void HINT_M_ALL::remove(const Record &rec)
{
    vector<pair<Timestamp, Timestamp> >::iterator iter, iterStart, iterEnd;
    vector<Timestamp>::iterator iterSStart, iterSEnd;
    vector<Timestamp>::iterator iterEStart, iterEEnd;
    ...
    for (auto l = 0; l < this->numBits; l++)
    {
        ...
        this->scanFirstPartitionDelete_OrgsIn(l, rec.id, a, rec.start, next_fromOinA);
        this->scanFirstPartitionDelete_OrgsOut(l, rec.id, a, next_fromOoutA);
        for (int j = a+1; j <= b; j++){
            this->scanPartitionsDelete_RepsOut(l, rec.id, j, next_fromRoutA);
            this->scanPartitionsDelete_RepsIn(l, rec.id, j, next_fromRinA);
        }
        a >>= 1; // a = a div 2
        b >>= 1; // b = b div 2
    }
    ...
    // Handle root.
    for (auto iter = this->pOrgsInIds[this->numBits].begin(); iter != this->pOrgsInIds[this->numBits].end(); iter++)
        if((*iter) == rec.id)
            (*iter)=0;
}
