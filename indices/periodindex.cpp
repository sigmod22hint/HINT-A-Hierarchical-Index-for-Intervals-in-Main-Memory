...
void AdaptivePeriodIndex::insert(const Record &rec)
{
    int level = 0;
    int sizeBucket = 0;
    int pos=0;
...
void AdaptivePeriodIndex::remove(const Record &rec)
{
    int level = 0;
    int sizeBucket = 0;
    int pos= 0;
    ...
    int startValue = rec.start;
    int endValue = rec.end;
    ...
    for(int i = 0; i < this->numLevels; ++i){
        ...
        for(int j = pos; j < pow(2,i) * this->numBuckets; ++j){
            for(int k = 0; k < this->bucketcounters[i][j]; ++k){
                if(this->buckets[i][j][k].id == rec.id){
                    this->buckets[i][j][k].id = 0;
                }
            }
            if(rec.end < this->endCounters[i][j])
                break;
        }
    }
}
