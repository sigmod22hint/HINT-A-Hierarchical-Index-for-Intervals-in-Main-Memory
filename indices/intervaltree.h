...
public:
    ...
    void insert(Interval<int, RecordId> ival,
            std::size_t depth = 16,
            std::size_t minbucket = 64,
            std::size_t maxbucket = 512, 
            Scalar leftextent = 0,
            Scalar rightextent = 0)
    {
            typename std::vector<Interval<Scalar, Value> >::iterator bound;
            depth--;
            interval_vector lefts;
            interval_vector rights;

            if (depth == 0 || (intervals.size() < minbucket && intervals.size() < maxbucket)) {
                bound = std::lower_bound(this->intervals.begin(), this->intervals.end(), ival, IntervalStartCmp());
                this->intervals.insert ( bound , ival );
                // this->intervals.push_back(ival);
                // std::sort(intervals.begin(), intervals.end(), IntervalStartCmp());
                return;
            }

            Scalar    leftp = intervals.front().start;
            Scalar    rightp = std::max_element(intervals.begin(), intervals.end(),
                                          IntervalStopCmp())->stop;
            

                if (ival.stop < center) {
                    
                    //std::sort(this->lefts.begin(), this->lefts.end(), IntervalStartCmp());
                    if(left){
                        left->insert(ival);
                    }
                    else{
                        lefts.push_back(ival);
                        left.reset(new IntervalTree(std::move(lefts), 
                                            depth, minbucket, maxbucket,
                                            leftp, center));
                    }

                } else if (ival.start > center) {
                    
                    //std::sort(this->rights.begin(), this->rights.end(), IntervalStartCmp());
                    if(right){
                        right->insert(ival);
                    }
                    else{
                        rights.push_back(ival);
                        right.reset(new IntervalTree(std::move(rights), 
                                             depth, minbucket, maxbucket, 
                                             center, rightp));
                    }
                    ...
                    // assert(ival.start <= center);
                    // assert(center <= ival.stop);
                    bound = std::lower_bound(this->intervals.begin(), this->intervals.end(), ival, IntervalStartCmp());
                    this->intervals.insert ( bound , ival );
                    // this->intervals.push_back(ival);
                    // std::sort(intervals.begin(), intervals.end(), IntervalStartCmp());
    ...
    void visit_near(const Scalar& start, const Scalar& stop, UnaryFunction f) {
    ...
    template <class UnaryFunction>
    void visit_overlapping(const Scalar& pos, UnaryFunction f) {
        visit_overlapping(pos, pos, f);
    }

    // Call f on all intervals overlapping [start, stop]
    template <class UnaryFunction>
    void visit_overlapping(const Scalar& start, const Scalar& stop, UnaryFunction f)  {
        auto filterF = [&]( interval& interval) {
    ...
    */
    size_t executeCount(const Scalar& start, const Scalar& stop) const {
        size_t result=0;
        visit_overlapping(start, stop,
                          [&](const interval& interval) { 
                            result++; 
    ...
    size_t executeCount(const Scalar& point) const {
        size_t result=0;
        visit_overlapping(point,
                          [&](const interval& interval) {
                              result++;
                          });
        return result;
    ...
    size_t execute(const Scalar& point)  {
        size_t result=0;
        visit_overlapping(point,
                          [&](const interval& interval) {
    ...
    size_t execute(const Scalar& start, const Scalar& stop) {
        size_t result=0;
        visit_overlapping(start, stop,
                          [&](const interval& interval) {
                              result^=interval.value;
    ...
    void remove(const Scalar& start, const Scalar& stop, RecordId id)  {
        size_t result=0;
        visit_overlapping(start, stop,
                          [&](interval& interval) {
